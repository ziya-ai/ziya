...
export function initMermaidEnhancer(): void {
  ...
  registerPreprocessor(
    (definition: string, diagramType: string): string => {
      if (!definition.trim().startsWith('quadrantChart') && !definition.trim().startsWith('quadrant-beta')) {
        return definition;
      }

      console.log('🔍 QUADRANT-FIX: Processing quadrant chart syntax');

      let result = definition;

      // Fix point syntax - ensure proper array format
      result = result.replace(/"([^"]+)":\s*\[([^\]]+)\]/g, (match, label, coords) => {
  ...
  // Add preprocessor for journey diagrams
  registerPreprocessor(
    (definition: string, diagramType: string): string => {
      if (!definition.trim().startsWith('journey')) {
        return definition;
      }

      console.log('🔍 JOURNEY-FIX: Processing journey diagram syntax');
      ...
      result = result.replace(/^(\s*)(.+?):\s*(\d+):\s*(.+)$/gm, (match, indent, taskName, score, participants) => {
        // Clean up participants list
        const cleanParticipants = participants.split(',').map(p => p.trim()).join(', ');
        return `${indent}${taskName}: ${score}: ${cleanParticipants}`;
      });

      // Ensure proper section structure with consistent indentation
      const lines = result.split('\n');
      const processedLines = lines.map(line => {
        const trimmed = line.trim();
        if (trimmed.startsWith('section ')) {
          return `    ${trimmed}`;
        } else if (trimmed && !trimmed.startsWith('journey') && !trimmed.startsWith('title')) {
          return `      ${trimmed}`;
        }
        return line;
      });
      ...
      console.log('🔍 JOURNEY-FIX: Processing complete');
      return result;
    }, {
  ...
  registerPreprocessor(
    (definition: string, diagramType: string): string => {
      ...
      // Handle subgraph with spaces in names - need to quote them
  ...
  registerPreprocessor(
    (definition: string, diagramType: string): string => {
      ...
      console.log('🔍 PIPE-SYNTAX-FIX: Processing flowchart pipe and arrow syntax');
      ...
      // Fix standalone connections with labels: "-- Label -->" should be "-->|Label|"
      result = result.replace(/--\s+([^-\n]+?)\s+-->/g, '-->|$1|');

      console.log('🔍 PIPE-SYNTAX-FIX: Processing complete');
  ...
  // Add a preprocessor to enhance packet diagram colors for dark mode
  registerPreprocessor(
    (definition: string, diagramType: string): string => {
      if (!definition.trim().startsWith('packet')) {
        return definition;
      }

      console.log('🎨 PACKET-DARK-MODE-PREP: Adding dark mode color enhancements to packet diagram');

      // Note: Mermaid packet diagrams don't support inline color customization in the definition
      // The actual color fixes need to happen post-render via SVG manipulation
      // This preprocessor just validates the definition is well-formed

      return definition;
    }, {
    name: 'packet-dark-mode-prep',
    priority: 120,
    diagramTypes: ['packet', 'packet-beta']
  });

  // Register a default error handler specifically for packet diagrams
  registerErrorHandler((error: Error, context: ErrorContext) => {
    if (!context.definition.trim().startsWith('packet')) {
      return false; // Not a packet diagram, let other handlers deal with it
    }

    console.log('🎨 PACKET-ERROR: Packet diagram rendering failed:', error.message);

    // Let the default handler show the error
    return false;
  }, {
    name: 'packet-error-handler',
    priority: 100,
    errorTypes: ['*']
  });
...
/**
 * Enhance packet diagram visibility in dark mode (called post-render)
 * @param svgElement - The SVG element containing the packet diagram
...
export function enhancePacketDarkMode(svgElement: SVGElement, isDarkMode: boolean): void {
  ...
  rects.forEach((rect) => {
    ...
    if (isDarkMode) {
      (rect as SVGElement).style.setProperty('fill', '#3b4252', 'important');
      (rect as SVGElement).style.setProperty('stroke', '#88c0d0', 'important');
    } else {
      (rect as SVGElement).style.setProperty('fill', '#ffffff', 'important');
      (rect as SVGElement).style.setProperty('stroke', '#333333', 'important');
    }
  ...
  textElements.forEach(textEl => {
    ...
    if (isDarkMode) {
      (textEl as SVGElement).style.setProperty('fill', '#eceff4', 'important');
    } else {
...
export default function initMermaidSupport(mermaidInstance?: any): void {
  ...
  registerPreprocessor((def: string, type: string) => {
    ...
    // Handle ER diagrams which use similar relationship syntax
    if (def.trim().startsWith('erDiagram')) {
      console.log('🔍 ER-DIAGRAM-FIX: Fixing ER diagram relationship syntax');
      // Fix ||--| syntax - this is invalid in ER diagrams, convert to ||--|| (one-to-one)
      result = result.replace(/\|\|--\|(?!\|)/g, '||--||');

      // Fix other invalid ER relationship patterns
      result = result.replace(/\|\|-->/g, '||--||');  // Convert ||--> to one-to-one
      result = result.replace(/--\|\|/g, '||--||');   // Convert --|| to one-to-one

      console.log('🔍 ER-DIAGRAM-FIX: Fixed invalid ER relationship patterns to ||--|| (one-to-one)');
      return result;
    }
