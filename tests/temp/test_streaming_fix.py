...
Note: StreamingMiddleware.safe_stream no longer emits a [DONE] sentinel.
Chunk count assertions reflect the actual chunks yielded by the generator.
"""
...
async def test_streaming_fix():
    ...
    # safe_stream yields only the chunks from the generator (no [DONE] sentinel)
    assert len(results) == 3  # 3 text chunks
