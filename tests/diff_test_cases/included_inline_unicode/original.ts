...
export const vegaLitePlugin: D3RenderPlugin = {
  ...
  render: async (container: HTMLElement, d3: any, spec: VegaLiteSpec, isDarkMode: boolean): Promise<void> => {
    ...
    try {
      ...
      // Fix radar chart specifications that use incorrect polar encoding
      if (vegaSpec.encoding?.theta && vegaSpec.encoding?.radius) {
        // Convert theta/radius to proper x/y coordinates for radar chart
        const dataValues = vegaSpec.data?.values || [];
        if (dataValues.length > 0) {
          // Transform data to cartesian coordinates
          vegaSpec.data.values = dataValues.map(d => ({
            ...d,
          ...
          vegaSpec.encoding = {
            ...
            y: { field: "y_coord", type: "quantitative", scale: { domain: [-100, 100] } },
            order: { field: "order" }
          };
        }
      }
