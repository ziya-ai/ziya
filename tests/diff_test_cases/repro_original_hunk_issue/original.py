...
def _repair_json_arguments(args_str: str) -> str:
    ...
    """
    # First, check if we're dealing with a shell command in the "command" field
    command_match = re.search(r'"command"\s*:\s*"(.*?)(?<!\\)"(?=,|\s*})', args_str)
    if command_match:
        # Extract the command value
        command_value = command_match.group(1)
        
        # Check if the command contains unescaped quotes or other problematic characters
        if ('"' in command_value or "'" in command_value or 
            '\\n' in command_value or '\\t' in command_value):
            
            # Create a properly escaped version of the command
            escaped_command = command_value.replace('\\', '\\\\')  # First escape backslashes
            escaped_command = escaped_command.replace('"', '\\"')  # Then escape double quotes
            
            # Replace the original command with the escaped version
            args_str = args_str[:command_match.start(1)] + escaped_command + args_str[command_match.end(1):]
            
            # Log the repair
            logger.debug(f"🔧 REPAIR: Fixed shell command escaping: {command_value[:50]}...")
