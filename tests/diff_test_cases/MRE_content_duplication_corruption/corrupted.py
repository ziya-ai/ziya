...
from .diff_pipeline import DiffPipeline, PipelineStage, HunkStatus, PipelineResultdef clean_duplicate_headers(diff_content: str) -> str:    """    Clean up duplicate header lines in a diff that can cause 'trailing garbage' errors.        Args:        diff_content: The diff content to clean            Returns:        The cleaned diff content    """    lines = diff_content.splitlines()    cleaned_lines = []    seen_headers = set()        for line in lines:        # Skip duplicate +++ or --- lines        if line.startswith('+++ ') or line.startswith('--- '):            if line in seen_headers:                continue            seen_headers.add(line)                cleaned_lines.append(line)        return '\n'.join(cleaned_lines)
