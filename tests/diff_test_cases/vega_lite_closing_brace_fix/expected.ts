...
export const vegaLitePlugin: D3RenderPlugin = {
  ...
  render: async (container: HTMLElement, d3: any, spec: VegaLiteSpec, isDarkMode: boolean): Promise<void> => {
    ...
    const preprocessVegaSpec = (rawSpec: any): any => {
      ...
      // Fix 0.6: If fold transform now has 'as' parameter, ensure encodings use the correct field names
      if (spec.transform && spec.transform.some((t: any) => t.fold && t.as)) {
        const foldTransform = spec.transform.find((t: any) => t.fold && t.as);
        if (foldTransform && spec.encoding) {
          const [keyField, valueField] = foldTransform.as;
          console.log(`🔧 FOLD-ENCODING-FIX: Fold transform outputs ["${keyField}", "${valueField}"], checking encodings...`);

          // Get all fields that exist in the data after folding
          const dataFields = new Set(Object.keys(spec.data?.values?.[0] || {}));
          dataFields.delete(foldTransform.fold[0]); // Remove folded fields
          dataFields.delete(foldTransform.fold[1]);
          dataFields.add(keyField); // Add fold output fields
          dataFields.add(valueField);

          console.log('🔧 FOLD-ENCODING-FIX: Available fields after fold:', Array.from(dataFields));

          // Check each encoding channel and fix references to non-existent fields
          Object.keys(spec.encoding).forEach(channel => {
            const channelSpec = spec.encoding[channel];
            if (channelSpec?.field && !dataFields.has(channelSpec.field)) {
              console.log(`🔧 FOLD-ENCODING-FIX: Channel "${channel}" references non-existent field "${channelSpec.field}"`);
              
              // If the field name suggests it should be the value field, update it
              if (channelSpec.field.includes('value') || channelSpec.field.includes('population') || 
                  channelSpec.field.includes('amount') || channelSpec.field.includes('score')) {
                console.log(`🔧 FOLD-ENCODING-FIX: Updating "${channel}" to use fold value field "${valueField}"`);
                channelSpec.field = valueField;
              }
              // If it suggests it should be the key field, update it
              else if (channelSpec.field.includes('key') || channelSpec.field.includes('category') || 
                       channelSpec.field.includes('type') || channelSpec.field.includes('gender')) {
                console.log(`🔧 FOLD-ENCODING-FIX: Updating "${channel}" to use fold key field "${keyField}"`);
