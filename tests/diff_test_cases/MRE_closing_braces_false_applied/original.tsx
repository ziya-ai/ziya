...
const MUIChatHistory = () => {
  ...
  const endCustomDrag = useCallback(async (dropTargetId?: string) => {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      // Show insertion line instead of highlighting the item
      ...
      if (treeItemBelow && treeItemBelow instanceof HTMLElement) {
        ...
        // Position the line above the target item
        ...
        if (containerRect) {
          insertionLine.style.top = (rect.top - containerRect.top - 2) + 'px';
    ...
    const handleMouseUp = (e: MouseEvent) => {
      if (customDragState.isDragging) {
        ...
        if (dropTarget) {
          ...
          // Find matching node in our tree data by text content
          ...
          if (matchingConversation) {
            targetNodeId = `conv-${matchingConversation.id}`;
            console.log('🎯 Found conversation target:', targetNodeId);
          } else {
            // Search in folders
            const matchingFolder = folders.find(folder => 
              folder.name.trim() === dropTargetText || 
              `${folder.name}(${conversations.filter(c => c.folderId === folder.id).length})`.trim() === dropTargetText
            );
            if (matchingFolder) {
              targetNodeId = matchingFolder.id;
              console.log('🎯 Found folder target:', targetNodeId);
            }
          }
        }
        
        console.log('🎯 Final target resolution:', { targetNodeId, draggedId: customDragState.draggedNodeId });
        endCustomDrag(targetNodeId);
      }
    };
