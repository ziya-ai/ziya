 ...
 * object (TaskScope), never at the block's top level.  Putting model_tier
 * on the block instead of in scope is a common authoring slip: the
 * ModelTierPicker reads scope.model_tier, and the block model allows extra
 * fields, so a block-level value is silently kept as dead junk and the
 * block runs on the inherited model.  liftBlockModelFields repairs it.
 */
 ...
 * Also lifts any misplaced block-level model field into scope (see
 * liftBlockModelFields) so scope.model_tier is populated where the
 * picker reads it, rather than rendering as "Inherit".
 */
