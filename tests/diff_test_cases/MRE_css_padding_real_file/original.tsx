...
import { Button, message, Space, Spin, Tooltip, Segmented } from 'antd';
                ...
                <>
                    <Tooltip title="View Type">
                        <Segmented
                            ...
                            value={viewType}
                            onChange={(value) => onViewTypeChange(value as 'unified' | 'split')}
                            size="small"
                            style={{
...
const DiffView = React.memo(function DiffView({ diff, viewType, initialDisplayMode, showLineNumbers, elementId, fileIndex }: DiffViewProps) {
    ...
    // Debug log when viewType changes
    useEffect(() => {
        console.log(`DiffView ${elementId} received viewType: ${viewType}`);
        window.diffViewType = viewType; // Ensure global setting is updated
    }, [viewType, elementId]);
                                ...
                                <tr className="hunk-content-wrapper">
                                    <td colSpan={viewType === 'split' ? 4 : 3} style={{
                                        ...
                                        borderLeft: status ? `3px solid ${isApplied ?
                                            (isAlreadyApplied ? '#faad14' : '#52c41a') :
                                            '#ff4d4f'}` : 'none',
                                        borderRight: status ? `1px solid ${isApplied ?
                                            (isAlreadyApplied ? '#faad14' : '#52c41a') :
                                            '#ff4d4f'}` : 'none',
                                        borderBottom: status ? `1px solid ${isApplied ?
                                            (isAlreadyApplied ? '#faad14' : '#52c41a') :
                                            '#ff4d4f'}` : 'none',
                                        borderRadius: '3px',
                                        overflow: 'hidden'
                                    }}>
                                        <table style={{ width: '100%', minWidth: 'max-content', borderCollapse: 'collapse', tableLayout: 'auto' }}><tbody>
    ...
    const styles = `
        ...
        /* Add horizontal scrolling at the hunk level */
        ...
        /* Force override any inline styles causing scrollbars on individual lines */
        .diff-line-content.token-container {
            overflow: visible !important;
        }
        
        /* Ensure all diff code cells don't have scrollbars */
        .diff-code {
            overflow: visible !important;
        }
    ...
    const renderFile = useCallback((file: any, fileIndex: number) => {
        return (
            <div
                ...
                data-file-title={renderFileHeader(file)}
            >
                <div className="diff-header" style={{ height: 'auto', overflow: 'visible' }}>
                    <div className="diff-header-content" style={{
                        display: 'flex',
                        flexWrap: 'wrap',
                        justifyContent: 'space-between'
                    }}>
                        {/* Add the hunk status indicators with flex-wrap */}
    ...
    return (
        <div className="diff-files-container">
            ...
            <style key={`diff-scroll-styles-${diffId}`}>{`
                /* Enable horizontal scrolling at hunk level */
                .hunk-content-wrapper {
                    overflow-x: auto !important;
                }
                
                /* Ensure table fills the hunk width */
                .hunk-content-wrapper > table {
                    width: 100%;
                    min-width: max-content;
                }
                
                /* Fix line content overflow */
                .diff-line-content {
                ...
                /* Style scrollbars */
                .hunk-content-wrapper::-webkit-scrollbar {
                    height: 8px;
                }
                
                .hunk-content-wrapper::-webkit-scrollbar-thumb {
...
const DiffViewWrapper = memo(({ token, enableCodeApply, index }: DiffViewWrapperProps) => {
    ...
    const content = (
        <div className="diff-view-wrapper" style={{
            position: 'relative',
            ...
            <DiffControls
                ...
                onViewTypeChange={(type) => {
                    window.diffViewType = type; // Update global setting immediately
                    setViewType(type);
                }}
...
export const MarkdownRenderer: React.FC<MarkdownRendererProps> = memo(({ markdown, enableCodeApply, isStreaming: externalStreaming = false }) => {
    ...
    useEffect(() => {
        ...
        if (true || !isStreamingState || previousTokensRef.current.length === 0) {
