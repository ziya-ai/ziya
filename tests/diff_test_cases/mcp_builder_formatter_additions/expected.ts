...
function formatAmazonTaskeiTasks(result: any, options: any): FormattedOutput {
  ...
  const formattedTasks = tasks.map((task: any, index: number) => {
    ...
    if (task.room && task.room.name) {
      taskInfo += `   • Room: ${task.room.name}\n`;
    }
    
    if (task.labels && task.labels.length > 0) {
      taskInfo += `   • Labels: ${task.labels.join(', ')}\n`;
    }
...
function formatAmazonPipelineHealth(result: any, options: any): FormattedOutput {
  ...
  const formattedPipelines = pipelines.map((pipeline: any) => {
    ...
    const badgeIcon = badge === 'gold' ? '🥇' : badge === 'silver' ? '🥈' : badge === 'bronze' ? '🥉' : '⚪';
    ...
    pipelineInfo += `• Builds: ${failedBuilds > 0 ? '❌' : '✅'} ${failedBuilds} failed\n`;
    pipelineInfo += `• Deployments: ${failedDeploys > 0 ? '❌' : '✅'} ${failedDeploys} failed`;
    if (failedProdDeploys > 0) pipelineInfo += ` (${failedProdDeploys} in prod)`;
    pipelineInfo += '\n';
    
    if (pendingApprovals > 0) {
      pipelineInfo += `• Manual Approvals: ⏸️ ${pendingApprovals} pending\n`;
    }
    
    if (isBlocked) {
      pipelineInfo += `\n⚠️ Pipeline is blocked and requires operator intervention\n`;
    }
