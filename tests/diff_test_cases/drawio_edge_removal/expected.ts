    ...
    const attemptRender = async () => {
        ...
        `;
            ...
            try {
                ...
                console.log('📐 DrawIO: Applying connection point fixes to edges');
                edgeCells.forEach(({id, element}) => {
                    const cell = cellMap.get(id);
                    if (!cell) return;
                    ...
                    if (currentStyle &&
                        ...
                        console.log(`📐 DrawIO: Edge ${id} already has connection points, skipping`);
                        return;
                    }
                ...
                console.log('📐 DrawIO: Post-processing edges for connection point fixes');
                edgeCells.forEach(({id, element}) => {
                    ...
                    if (isBidirectional) {
                        ...
                        if (sourceGeom && targetGeom) {
                            ...
                            cell.setStyle(currentStyle);
                            console.log(`📐 DrawIO: POST-PROCESS: Offset bidirectional edge ${id} by ${offset}`);
                        }
                    } else {
                        ...
                        if ((isSourceRectangle && sourceHasBoth) || (isTargetRectangle && targetHasBoth)) {
                            ...
                            if (sourceGeom && targetGeom) {
                                ...
                                cell.setStyle(currentStyle);
                                console.log(`📐 DrawIO: POST-PROCESS: Separated in/out for rectangle edge ${id}`);
                            }
                        }
                    }
                });
