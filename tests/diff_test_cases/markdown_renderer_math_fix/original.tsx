...
const MathRenderer: React.FC<{ math: string; displayMode: boolean }> = ({ math, displayMode }) => {
    ...
    try {
        console.log('🔍 KaTeX rendering:', {
            math: math.substring(0, 100),
            displayMode,
            hasMathbf: math.includes('mathbf')
        });
        ...
        // Handle math errors and render as plain text instead
        console.error('❌ KaTeX rendering error:', error, 'Math content:', math);
        return displayMode ?
            <div className="math-fallback" style={{ fontFamily: 'monospace', padding: '4px' }}>{math}</div> :
            <span className="math-fallback" style={{ fontFamily: 'monospace' }}>{math}</span>;
    }
                    ...
                    processedMarkdown = segments.map((segment, index) => {
                        ...
                        try {
                            ...
                            processed = processed.replace(
                                /\$([^⟩$\n]+?)\$/g,
                                (match, p1) => {
                                    console.log('📐 INLINE MATH candidate:', { match, content: p1, hasMathbf: p1.includes('mathbf') });
