    ...
    return tokens.map((token, index) => {
        ...
        try {
            switch (determinedType) {
                ...
                case 'graphviz':
                    return (
                        <LazyD3Renderer key={index}
                            spec={{ type: "graphviz", definition: token.text }}
                        />
                    );

                case 'mermaid':
                    return <LazyD3Renderer key={index} spec={mermaidSpec} />;

                case 'tool':
                    if (isSecurityError) {
                        return (
                            <Alert key={index} message="Blocked" />
                        );
                    }
                    if (isThinking) {
                        return (
                    ...
                    return (
                        <ToolBlock key={index} toolName={name} content={toolContent} />
                    );
