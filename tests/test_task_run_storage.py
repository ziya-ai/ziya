...
class TestPauseResume:
    def test_request_pause_sets_flag_only(self, storage):
        ...
        # Flag only — status is flipped to "paused" by the executor at a
        # boundary, not by request_pause.
    ...
    def test_reconcile_sweeps_paused(self, storage):
        run = storage.create(TaskRunCreate(card_id="c"))
        # A paused run's sleeping coroutine dies on restart → zombie.
