...
| **Conversation export / import** | Export for local reuse (JSON), export for sharing (Markdown), server-side high-fidelity **PDF** (headless-rendered; falls back to browser print if Playwright is absent), export directly to paste services; import to restore |
...
| **Music notation** | Published-quality sheet music via VexFlow — inline `` `music:` `` phrases or a ```` ```music ```` JSON spec. Notes/chords/rests, 5 clefs, key/time signatures, beaming, tuplets, grace & cue notes, tremolo, arpeggios, slurs/ties/glissandos, dynamics/hairpins, articulations, ornaments, chord symbols, lyrics (verses/hyphens/melisma), fingerings, harp-pedal & sustain-pedal lines, breath marks/caesuras, cautionary accidentals, measures/barlines/repeats/voltas, navigation & tempo marks, title block & part names, multi-voice staves, grand staff with cross-staff beams/slurs, and automatic multi-system wrapping with measure numbering |
