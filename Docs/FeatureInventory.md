...
| **Model-staged cards are conversation-only** | When the model hands sub-tasks off with `task_card_stage`, the card is saved as an *unlisted* card: its inline tile, run and bindings all work, and the model can still find it (`task_card_list` with `bound_to_current_chat`), but it never appears in the Task Cards deck — so a session that fans out ten sub-tasks does not leave ten permanent artifacts behind. The tile shows a `conversation-only` tag with **Save to deck**; the deck editor offers **Add to deck** for such a card opened from a tile. Promotion is in place (ids and signatures survive). `persist: true` on the tool files the card in the deck up front. A card can never become unreachable: with no conversation to bind to it goes to the deck, and the stale-draft cleanup skips any card a conversation tile still points at |
