...
async def change_resolution(request: ResolutionChangeRequest) -> Dict[str, Any]:
    ...
    """
    try:
        from app.utils.ast_parser.integration import change_ast_resolution
        
        # Validate resolution level
        valid_resolutions = ['minimal', 'medium', 'detailed', 'comprehensive']
        ...
        def reindex_background():
            try:
                change_ast_resolution(resolution)
            except Exception as e:
        ...
        return {"status": "success", "message": f"AST resolution changed to {resolution}. Re-indexing in progress."}
    except Exception as e:
