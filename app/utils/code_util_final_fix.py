"""
Final fixes for the difflib implementation.
This script applies targeted fixes to the code_util.py file to address the remaining issues.
"""
...
# Fix 1: Improve is_hunk_already_applied to handle constant_duplicate_check
is_hunk_already_applied_fix = '''
...
# Fix 2: Improve apply_diff_with_difflib_hybrid_forced to handle alarm_actions_refactor
apply_diff_fix = '''
...
content = re.sub(
    r'def apply_diff_with_difflib_hybrid_forced\([^)]*\).*?(?=\n\w+\s*\w+|$)',
    apply_diff_fix,
    content,
    flags=re.DOTALL
)
...
print(f"Successfully applied final fixes to {code_util_path}")
